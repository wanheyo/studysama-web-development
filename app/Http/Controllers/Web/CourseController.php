...
class CourseController extends Controller
{
    ...
    public function course_detail(Request $request, $course_id)
    {
        ...
        return view('course.course_detail', compact('course', 'topics', 'user_courses', 'tutor', 'tutor_follow', 'tutor_courses', 'course_comments', 'user_review', 'joined_users', 'courseProgress', 'totalLessons', 'totalResources', 'totalChecked', 'totalComments', 'totalCompletedLessons', 'totalCompletedResources', 'claimedCertificate', 'certificate'));
    }
    ...
    public function course_tutor_statistics(Request $request, $course_id)
    {
        ...
        $latestResourceDate = Resource::where('status', 1)
    ...
    public function course_certificate(Request $request, $course_id, $user_course_id = null)
    {
        ...
        return view('course.certificate', compact('course', 'userCourse', 'certificate', 'courseProgress'));
    }
    ...
    public function show_course_certificate($certificate_id)
    {
        ...
        $lessons = Lesson::with(['resources' => function ($query) use ($userCourse) {
            $query->where('status', 1)
            ->with(['resourceFile' => function ($query) {
                $query->where('status', 1);
            }])
            ...
            ->with(['userProgressions' => function ($query) use ($userCourse) {
                $query->whereHas('userCourse', function ($subQuery) use ($userCourse) {
                    $subQuery->where('id', $userCourse->id)
                            ->where('status', 1);
                });
            }]);
        }])
